...
if "__main__":
    image_to_hide_path = "./assets/default_cover.png"
    image_to_hide_in_path = "./download.png"
    encoded_image_path = "./encoded_image.png"
    decoded_image_path = "./decoded_image.png"
    n_bits = 2

    # Open images and convert to RGB mode
    image_to_hide = Image.open(image_to_hide_path).convert("RGB")
    image_to_hide_in = Image.open(image_to_hide_in_path).convert("RGB")

    # Encode and save the encoded image
    encode(image_to_hide, image_to_hide_in, n_bits).save(encoded_image_path)

    # Decode the image (if needed)
    # image_to_decode = Image.open(encoded_image_path).convert("RGB")
